...
export default function InfoPopup({ infoDataState, setInfoDataState }) {
  ...
  return (
    <div className="w-full select-none info-box bg-blue-500/5 backdrop-blur-xs transition-all duration-300 absolute inset-0 flex items-center justify-center z-[88]">
      <div className="min-h-[60vh] relative h-[60vh] bg-white rounded-xl shadow-lg w-[25%] p-[1.5vw] flex flex-col justify-between">
        <span
          onClick={handleClose}
          className="text-gray-700 hover:text-black text-xl absolute right-0 top-0 p-[.5vw]"
        >
          <svg
        ...
        <div className="info-popup-content flex-1 py-3">
          ...
          >
            <p className="text-black text-sm">{openedInfo.description}</p>
          </div>
        </div>

        {/* Controls - Arrows + Progress */}
        <div className="flex items-center justify-evenly mt-[1vw] gap-4">
          <button
            onClick={handleLeft}
          ...
          <div className="flex gap-2">
            {infoDataState.map((_, i) => (
              <span
                key={i}
                className={`h-[.15vw] w-[1.5vw] rounded-full transition-all duration-300 ${
